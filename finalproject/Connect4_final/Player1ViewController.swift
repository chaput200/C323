...
class Player1ViewController: UIViewController {
    ...
    @IBAction func nextPlayer(_ sender: UIButton){
        //SET VIEW UP FOR NEXT PLAYER INFO
        if let name = self.nameTextField?.text{
            self.gameSession?.setPlayer1(name: name, color: "red")
        }
        //print("P1VC P1 INFO: \(self.gameSession?.p1Name.description) \(self.gameSession?.p1Color)\n")
    }
